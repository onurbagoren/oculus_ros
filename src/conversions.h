...
namespace oculus
{
    ...
    inline void copy_to_ros(sensor_msgs::CompressedImage &msg,
                            const oculus::PingMessage::ConstPtr &ping)
    {
        ...
        // ── Image geometry & OpenCV type ─────────────────────────────────────────
        const int sample_size = ping->sample_size(); // 1 = 8-bit, 2 = 16-bit
        int src_cols = ping->bearing_count();
        if (ping->has_gains())
            src_cols += (sample_size == 1 ? 4 : 2); // gain columns

        const int src_rows = ping->range_count();

        const int cv_type = (sample_size == 1) ? CV_8UC1 : (sample_size == 2) ? CV_16UC1
                                                                              : CV_8UC1; // fallback

        // Wrap the sonar frame without extra copies
        cv::Mat full(src_rows,
                     src_cols,
                     cv_type,
                     const_cast<uint8_t *>(ping->ping_data()),
                     ping->step());

        // ── Down-sample to ¼ resolution (½ × ½) ─────────────────────────────────
        cv::Mat downsampled;
        cv::resize(full,
                   downsampled,
                   cv::Size(src_cols / 2, src_rows / 2),
                   0, 0,
                   cv::INTER_NEAREST);
        ...
        cv::imencode(".png",
                     downsampled,
                     buffer,
                     {cv::IMWRITE_PNG_COMPRESSION, 3}); // fast, light compression

        msg.format = (sample_size == 1 ? "mono8" : "mono16") + std::string("; png");
        msg.data.swap(buffer);
    }
